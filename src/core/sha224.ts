...
export function sha224Bytes(input: Uint8Array): Uint8Array {
  const h = [
  ...
  // 64-bit big-endian length (high 32 bits stay 0 for our input sizes)
  ...
  for (let off = 0; off < totalLen; off += 64) {
    ...
    h[2] = (h[2] + c) >>> 0; h[3] = (h[3] + d0) >>> 0;
    h[4] = (h[4] + e) >>> 0; h[5] = (h[5] + f) >>> 0;
    h[6] = (h[6] + g) >>> 0; h[7] = (h[7] + h0) >>> 0;
  }
...
export function sha224Hex(text: string): string {
  const bytes = sha224Bytes(encoder.encode(text));
  let s = '';
